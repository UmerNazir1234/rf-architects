...
export default function ProjectsClient({ initialProjects }: ProjectClientProps) {
  const [page, setPage] = useState(0);
  const projectsPerPage = 6;
  const totalPages = Math.ceil(initialProjects.length / projectsPerPage);

  const visibleProjects = initialProjects.slice(
    0,
        ...
        <div className="container mx-auto px-6 md:px-8">
          {initialProjects.length === 0 ? (
            ...
            <>
              ...
              {/* Load More Button */}
              {visibleProjects.length < initialProjects.length && (
